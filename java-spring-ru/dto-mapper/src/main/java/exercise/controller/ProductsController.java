...
public class ProductsController {
    ...
    @GetMapping("/{id}")
    public ProductDTO getAllProducts(@PathVariable Long id) {
    ...
    @PutMapping("/{id}")
    public ProductDTO updateProduct(@PathVariable Long id, @RequestBody ProductUpdateDTO productUpdDTO) {
        var product = productRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException(""));

        productMapper.update(productUpdDTO, product);
        productRepository.save(product);
